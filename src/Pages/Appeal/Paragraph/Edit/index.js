...
const ParagraphEdit = () => {
  ...
  const { appealParagraph, loading, error, success } = useSelector(
    (state) => state.appeal
  );

  useEffect(() => {
    dispatch(getAppealPargraphByID(accessToken, id));
  }, []);

  useEffect(() => {
    if (success && success.type == "appeal_paragraph_update_success") {
      toast.success("Appeal Paragraph Updated");
  ...
  useEffect(() => {
    paragraphForm.setFieldsValue({
      p_title: appealParagraph?.title,
      p_body: appealParagraph?.body,
      p_serial_number: appealParagraph?.serial_number,
    });
  }, [appealParagraph]);
  ...
  const onParagraphSubmit = (fieldsValue) => {
    ...
    let modelId = appealParagraph?.model_id;
    dispatch(updateAppealParagraph(accessToken, data, id, modelId));
  };
